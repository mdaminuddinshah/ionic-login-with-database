...
export class NetworkService {
  ...
  public registerII(name:string, email:string, password:string){
    return new Promise((resolve, reject)=>{
      try{
        const detailsII = {
          url: "https://namdosan.online/api/auth/register",
        ...
        CapacitorHttp.post(detailsII).then(
          (res)=>{
          resolve(res);
        // }, 
        // (err)=>{
  ...
  register3(name:string, email: string, password: string){
    return new Promise((resolve, reject)=>{
      try{const details = {
        url: 'https://namdosan.online/api/auth/register',
        headers: {'Content-Type': 'application/json'},
      ...
      CapacitorHttp.post(details).then((res)=>{
        resolve(res);
      })
    } catch (err){
      reject(err);
  ...
  // register4 part
  register4(name:string, email:string, password:string){
    return new Promise((resolve, reject)=>{
      try{
        const details = {
          url: 'https://namdosan.online/api/auth/register',
  ...
  // part register5
  public register5(name:string, email:string, password:string){
    return new Promise((resolve, reject)=>{
